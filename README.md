...
```bash
ml-app$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ml-app$ sam local start-api
ml-app$ curl http://localhost:3000/classify_digit
```
...
```bash
ml-app$ sam logs -n InferenceFunction --stack-name ml-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ml-app
```
